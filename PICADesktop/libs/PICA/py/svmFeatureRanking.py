...
def printFeatureRanking(w, dimRank, args):
    ...
    for rank in dimRank:
        ...
        if abs(w[rank]) >= relevanceThreshold:
            ...
            if len(featureGroup) > 1:
                ...
                ranking.append((",".join(featureGroup),str(w[rank]),predictorForClass,description))
            else:
    ...
    with open(args.output,"w") as rank_file:
...
def checkArguments(args):
    ...
    if args.descr and not os.path.isfile(args.descr):
        ...
        exit(1)
    if not args.output:
        args.output = ".".join(args.model.split(".")[:-1]) + ".rank"
    if args.output and not os.path.isdir(os.path.dirname(args.output)):
        print "ARGUMENT ERROR: parent directory of output file (-o) could not be found"
        exit(1)
...
parser.add_argument("-o", "--output", action="store", dest="output", metavar="FILE", help="Specify file-path for the output")
